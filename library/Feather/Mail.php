...
class Mail {
    ...
    * 设置发件人
    * @access public
    * @param string $username 发件人地址
    * @param string $alias 发件人名称
    * @return boolean
    */
    public function setFrom($username, $alias = '') {
        $this->_from = $username;
        if(!empty($alias)){
            $this->_alias = $alias;
        }else{
    ...
    */
    public function sendMail() {
        ...
        foreach ($command as $value) {
            if($this->_sendCommand($value[0], $value[1])) {
                continue;
            }
            else{
                return false;
    ...
    */
    protected function _getCommand() {
        ...
        //设置发件人
        $header = "FROM: ". $this->_alias ."<" . $this->_from . ">\r\n";
        ...
        if(isset($this->_attachment)){
            $header .= "\r\n--" . $separator . "\r\n";
            $header .= "Content-Type: " . $this->_getMIMEType() . '; name="' . basename($this->_attachment) . '"' . "\r\n";
            $header .= "Content-Transfer-Encoding: base64\r\n";
            $header .= 'Content-Disposition: attachment; filename="' . basename($this->_attachment) . '"' . "\r\n";
            $header .= "\r\n";
            $header .= $this->_readFile();
            $header .= "\r\n--" . $separator . "\r\n";
        }
    ...
    */
    protected function _sendCommand($command, $code) {
        $this->_debugInfo('Send command:' . $command . ',expected code:' . $code);
        //发送命令给服务器
        try{
    ...
    */
    ...
    private function _debugInfo($str){
        if($this->_isDebug){
            echo $str . "\n";
        }
    }
